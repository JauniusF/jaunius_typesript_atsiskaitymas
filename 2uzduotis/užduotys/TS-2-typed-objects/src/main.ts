...
{
  const isAdult = (j: Person): boolean => j.age >= 14;
...
{
  type HasWeightAndHeight=(p:Person)=>boolean;
  const isFullyDescribedPerson:HasWeightAndHeight = (Person)=> Boolean(Person.height) && Boolean(Person.weight)
...
{
  const createInitials = (p:Person) => {return `Inicialai: ${p.name[0]}${p.surname[0]}`};
